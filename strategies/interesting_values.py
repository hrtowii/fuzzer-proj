...
class InterestingValuesStrategy(MutationStrategy):
    ...
    def _substitute_integer(self, data: bytes) -> bytes:
        ...
        if size == 1:
            packed = (
                struct.pack("<b", interesting_value)
                if signed
                else struct.pack("<B", interesting_value)
            )
        elif size == 2:
            packed = (
                struct.pack("<h", interesting_value)
                if signed
                else struct.pack("<H", interesting_value)
            )
        elif size == 4:
            packed = (
                struct.pack("<i", interesting_value)
                if signed
                else struct.pack("<I", interesting_value)
            )
        else:  # size == 8
            packed = (
                struct.pack("<q", interesting_value)
                if signed
                else struct.pack("<Q", interesting_value)
            )
    ...
    def _substitute_string(self, data: bytes) -> bytes:
        ...
        encoded_string = (
            interesting_string.encode("utf-8")
            if isinstance(interesting_string, str)
            else interesting_string
        )
    ...
    def _mutate_specific_format(self, data: bytes, format_name: str) -> bytes:
        ...
        if format_name == "jpeg":
            ...
            if len(data) > 4:
                ...
                max_pos = max(2, min(10, len(result) - 1))
                pos = random.randint(2, max_pos)
            ...
            if len(data) > 8:
                ...
                if chunk_start + 4 < len(result):
                    max_pos = max(chunk_start, min(chunk_start + 8, len(result) - 1))
                    pos = random.randint(chunk_start, max_pos)
                    result[pos] = random.randint(0, 255)
                return bytes(result)
            ...
            if len(data) > 16:
                ...
                max_pos = max(4, min(16, len(result) - 1))
                pos = random.randint(4, max_pos)
                result[pos] = random.randint(0, 255)
                return bytes(result)
    ...
    def _generate_interesting_value(self) -> bytes:
        ...
        if category == "integers":
            subcategory = random.choice(
                list(self.interesting_values["integers"].keys())
            )
            values = self.interesting_values["integers"][subcategory]
            value = random.choice(values)
            return (
                struct.pack("<q", value)
                if "64bit" in subcategory
                else struct.pack("<i", value)
                if "32bit" in subcategory
                else struct.pack("<h", value)
                if "16bit" in subcategory
                else struct.pack("<b", value)
            )

        elif category == "floating_point":
            subcategory = random.choice(
                list(self.interesting_values["floating_point"].keys())
            )
            ...
            subcategory = random.choice(
                list(self.interesting_values["file_formats"].keys())
            )
